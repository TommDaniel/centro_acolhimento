...
<head>
<meta charset="utf-8">
<title>Relatório de Ocorrência — {{ $report->crianca->nome_completo }}</title>
<style>
...
<main>
    <table class="oficio">
        <tr>
            <td>Ofício nº {{ $report->numero_oficio ?? '—' }}</td>
    ...
    <table class="ident">
        <tbody>
            <tr>
                ...
                <td>{{ $report->crianca->nome_completo }}</td>
            </tr>
            @if($report->crianca->data_nascimento)
                <tr>
                    <td class="rotulo">Data de nascimento</td>
                    <td>{{ $report->crianca->data_nascimento->format('d/m/Y') }}</td>
                </tr>
            @endif
            @if($report->crianca->processo_numero)
                <tr>
                    <td class="rotulo">Nº do processo</td>
                    <td>{{ $report->crianca->processo_numero }}</td>
                </tr>
            @endif
        </tbody>
    </table>

    @foreach($report->secoes() as $titulo => $texto)
